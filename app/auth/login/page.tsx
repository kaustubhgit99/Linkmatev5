...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Wait for session to be fully set before querying profile
    // Use the user from the auth response directly — no need for extra round trip
    const userId = data.user.id

    // Retry profile fetch up to 3 times with a short delay
    // This handles the case where the DB trigger hasn't created the row yet
    let role = 'citizen'
        ...
        <div className="glass-card rounded-3xl p-8 border border-white/10 shadow-2xl">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="space-y-1.5">
              <Label>Email</Label>
              <Input type="email" placeholder="you@example.com" value={email} onChange={e=>setEmail(e.target.value)} required autoComplete="email" className="h-11"/>
            </div>
            <div className="space-y-1.5">
              <Label>Password</Label>
              <div className="relative">
                <Input type={show?'text':'password'} placeholder="••••••••" value={password} onChange={e=>setPassword(e.target.value)} required className="h-11 pr-10"/>
          ...
          <p className="text-center text-sm text-muted-foreground mt-6">
            No account? <Link href="/auth/signup" className="text-violet-500 hover:text-violet-400 font-medium">Sign up free</Link>
          </p>
        </div>
      </motion.div>
    </div>
